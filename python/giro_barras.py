import serial
import time
import pygame
import sys

# --- CONFIGURACIÓN SERIAL ---
PUERTO_SERIAL = 'COM5' 
VELOCIDAD_BAUDIOS = 9600
TIEMPO_ESPERA = 4 

# 🌟 UMBRAL DE CAMBIO: Solo imprime si la diferencia es >= 3
UMBRAL_CAMBIO = 3 

# --- CONFIGURACIÓN GRÁFICA ---
NUM_OPCIONES = 4 
VENTANA_ANCHO = 400
VENTANA_ALTO = 300
COLOR_FONDO = (0, 0, 0)       # Negro
COLOR_INACTIVO = (180, 180, 180) # Gris claro
COLOR_ACTIVO = (255, 0, 0)     # Rojo
POSICION_Y_BARRA = 50
ALTURA_BARRA = 150

# Variables de seguimiento
ultimo_valor_pot = None
ultima_opcion_activa = -1 

# Función para mapear el valor del potenciómetro (0-1023) a la opción (0 a 3)
def mapear_a_opcion(valor_pot):
    """Mapea un valor de potenciómetro (0-1023) a un índice de opción (0 a 3)."""
    # Rango de cada opción: 1024 / 4 = 256.
    opcion = valor_pot // (1024 // NUM_OPCIONES)
    return min(opcion, NUM_OPCIONES - 1)


# --- INICIO DE PYGAME Y CONEXIÓN SERIAL ---
try:
    # 1. Conexión Serial
    ser = serial.Serial(
        port=PUERTO_SERIAL,
        baudrate=VELOCIDAD_BAUDIOS,
        timeout=0.05 # Un timeout pequeño ayuda a que Pygame no se congele
    )
    ser.flushInput() 
    print(f"Esperando {TIEMPO_ESPERA} segundos para el inicio del Arduino...")
    time.sleep(TIEMPO_ESPERA) 
    print("Iniciando GUI con Pygame. Mueve el potenciómetro.")

    # 2. Inicializar Pygame
    pygame.init()
    screen = pygame.display.set_mode((VENTANA_ANCHO, VENTANA_ALTO))
    pygame.display.set_caption("Selector de Menú - Pygame")
    clock = pygame.time.Clock()
    font = pygame.font.Font(None, 24)
    
    ancho_segmento = VENTANA_ANCHO / NUM_OPCIONES # Ancho de cada barra

    running = True
    while running:
        # Manejo de eventos (cerrar ventana)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False

        # --- LECTURA SERIAL ---
        if ser.in_waiting > 0:
            linea_bytes = ser.readline()
            linea_string = linea_bytes.decode('utf-8').strip()
            
            if "Valor del Potenciómetro (A0):" in linea_string:
                try:
                    valor_str = linea_string.split(':')[-1].strip()
                    valor_numerico = int(valor_str)
                    
                    # Aplicar el umbral de cambio
                    if ultimo_valor_pot is None or abs(valor_numerico - ultimo_valor_pot) >= UMBRAL_CAMBIO:
                        
                        # Mapear el valor pot a la opción (0, 1, 2, o 3)
                        nueva_opcion = mapear_a_opcion(valor_numerico)
                        
                        # Actualizar solo si la opción ha cambiado
                        if nueva_opcion != ultima_opcion_activa:
                            ultima_opcion_activa = nueva_opcion
                            
                        ultimo_valor_pot = valor_numerico
                        
                except ValueError:
                    pass
        
        # --- DIBUJO DE LA INTERFAZ ---
        screen.fill(COLOR_FONDO)
        
        # Dibujar las 4 barras
        for i in range(NUM_OPCIONES):
            # 1. Definir la posición y tamaño de la barra
            rect_x = i * ancho_segmento
            rect_y = POSICION_Y_BARRA
            rect_w = ancho_segmento
            rect_h = ALTURA_BARRA
            
            # 2. Elegir el color (Rojo si es la activa, Gris si es inactiva)
            color = COLOR_ACTIVO if i == ultima_opcion_activa else COLOR_INACTIVO
            
            # 3. Dibujar el rectángulo
            pygame.draw.rect(screen, color, (rect_x, rect_y, rect_w, rect_h), border_radius=5)
            pygame.draw.rect(screen, (255, 255, 255), (rect_x, rect_y, rect_w, rect_h), 1) # Borde blanco
            
            # 4. Dibujar el texto de la opción
            text_surface = font.render(f"Opción {i+1}", True, (255, 255, 255))
            text_rect = text_surface.get_rect(center=(rect_x + ancho_segmento/2, rect_y + ALTURA_BARRA + 30))
            screen.blit(text_surface, text_rect)

        # 5. Mostrar la opción activa en la parte inferior
        if ultima_opcion_activa != -1:
            status_text = f"Opción Activa: {ultima_opcion_activa + 1} | Potenciómetro: {ultimo_valor_pot}"
        else:
            status_text = "Esperando datos del potenciómetro..."
            
        status_surface = font.render(status_text, True, (255, 255, 255))
        screen.blit(status_surface, (10, VENTANA_ALTO - 30))
        
        # Actualizar toda la pantalla
        pygame.display.flip()
        
        # Limitar la velocidad de fotogramas (frame rate) para no consumir demasiados recursos
        clock.tick(60)

    # 3. Al salir del bucle
    pygame.quit()

except serial.SerialException as e:
    print(f"\nERROR: No se pudo abrir el puerto serial {PUERTO_SERIAL}.")
    print("-----------------------------------------------------")
    print("CAUSA PROBABLE: El puerto está BLOQUEADO. Asegúrate de cerrar otros monitores seriales.")
    print(f"Detalle del error: {e}")

except Exception as e:
    print(f"\nERROR INESPERADO durante la ejecución de Pygame: {e}")
    
finally:
    # 4. Asegurar el cierre del puerto
    if 'ser' in locals() and ser.is_open:
        ser.close()
        print("\nPrograma finalizado y puerto serial cerrado.")